...
#define setbit(port, bit, val) do { if(val) (port) |= (1<<(bit)); else (port) &= ~(1<<(bit)); } while(0)

#define getInput() getbit(PIND, 0)

#define setOutput(b) setbit(PORTD, 1, (b))

#define set1(b) setbit(PORTB, 7, (b))
#define set2(b) setbit(PORTB, 1, (b))
