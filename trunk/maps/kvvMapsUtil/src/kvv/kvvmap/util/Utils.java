...
public class Utils {
	...
	public static double lon2x(double lon, int zoomLevel, int tileSize) {
		...
		return x * tileSize / TILE_SIZE_G;
	}

	public static double lat2y(double lat, int zoomLevel, int tileSize) {
		long size = (1L << zoomLevel) * TILE_SIZE_G; // - размер карты
																// на уровне
		...
		double y = center - 0.5 * Math.log((1 + ls) / (1 - ls))
				* (size / (2 * Math.PI)); // - координата y точки находящейся на
											// широте Lat;
		return y * tileSize / TILE_SIZE_G;
	}

	public static double x2lon(double x, int zoomLevel, int tileSize) {
		x = x * TILE_SIZE_G / tileSize;
		long size = (1L << zoomLevel) * TILE_SIZE_G; // - размер карты
																// на уровне
		// детализации zoomLevel в пикселях;
		long center = size / 2; // - задаёт x и y координаты центра карты в
	...
	public static double y2lat(double y, int zoomLevel, int tileSize) {
		y = y * TILE_SIZE_G / tileSize;
		long size = (1L << zoomLevel) * TILE_SIZE_G; // - размер карты
																// на уровне
		// детализации zoomLevel в пикселях;
		long center = size / 2; // - задаёт x и y координаты центра карты в
