...
public class ViewHelper {
	...
	private static double pt2m(int pt, MapViewParams mapPos) {
		double m1deg = 111000;
		double lat1 = Utils.y2lat(mapPos.centerY() - pt / 2, mapPos.getZoom(), Adapter.TILE_SIZE);
		double lat2 = Utils.y2lat(mapPos.centerY() + pt / 2, mapPos.getZoom(), Adapter.TILE_SIZE);
		return (lat1 - lat2) * m1deg;
	}

	private static double m2pt(double m, MapViewParams mapPos) {
		double m1deg = 111000;
		double lat1 = Utils.y2lat(mapPos.centerY() - 10 / 2, mapPos.getZoom(), Adapter.TILE_SIZE);
		double lat2 = Utils.y2lat(mapPos.centerY() + 10 / 2, mapPos.getZoom(), Adapter.TILE_SIZE);
