...
public final class MapViewParams {
	...
	public double loc2scrX(LocationX loc) {
		return geo2scrX(loc.getX(zoom), loc.getY(zoom));
	}

	public double loc2scrY(LocationX loc) {
		return geo2scrY(loc.getX(zoom), loc.getY(zoom));
	}
	...
	public double scr2lon(double x, double y) {
		return Utils.x2lon(scr2geoX(x, y), zoom, Adapter.TILE_SIZE);
	}

	public double scr2lat(double x, double y) {
		return Utils.y2lat(scr2geoY(x, y), zoom, Adapter.TILE_SIZE);
	}
	...
	public void scrollBy(double dx, double dy) {
		...
		double lon = Utils.x2lon(x, zoom, Adapter.TILE_SIZE);
		double lat = Utils.y2lat(y, zoom, Adapter.TILE_SIZE);
		if (lat > 85 || lat < -85)
			return;
		// centerX = x;
		// centerY = y;
