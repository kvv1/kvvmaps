...
//		}
	...
	private static void dHeapSort(double key[], double trail[]) {
		int nkey = key.length;
	...
	private static void dremake_heap(double key[], double trail[],
	...
	/*
	 * public static void main(String[] args) // no args expected { // FUNCTION:
	 * Driver for using cubic interpolatory spline routines. int n = 11;
	 * 
	 * double xx[] = new double[n]; double ff[] = new double[n]; double s, t;
	 * DecimalFormat f1 = new DecimalFormat("00.00000"); // Assign values for
	 * data arrays x and f.
	 * System.out.println("Spline.java function definition"); for (int i = 0; i
	 * < n; i++) { xx[i] = -2.0 + 0.40 * i; ff[i] = Math.pow(Math.cos(xx[i]),
	 * 4); System.out.println("i=" + i + ", x[i]=" + f1.format(xx[i]) +
	 * ", f(x[i])=" + f1.format(ff[i])); } // Calculate coefficients defining
	 * the cubic spline. Spline s11 = new Spline(xx, ff);
	 * System.out.println("interpolated values"); for (int i = 0; i < 41; i++) {
	 * t = -2.0 + 0.09999999999 * i; s = s11.spline_value(t);
	 * System.out.println("x=" + f1.format(t) + ",  f(x)=" + f1.format(s)); } t
	 * = s11.integrate(); System.out.println("integral=" + t);
	 * System.out.println("expect  =1.183433"); } // end main
	 */
} // end class Spline