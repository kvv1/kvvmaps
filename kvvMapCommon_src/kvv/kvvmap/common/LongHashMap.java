...
public class LongHashMap<T> {
	...
	private static class Entry<T> {
		 ...
		 */
		protected Entry(long hash, /* int key, */T value, Entry<T> next) {
			this.hash = hash;
			// this.key = key;
			this.value = value;
			this.next = next;
	...
	public boolean containsKey(long key) {
		Entry<T> tab[] = table;
		long hash = key;
		int index = hash(hash, tab.length);
	...
	public long[] keys() {
		if (count == 0)
			...
			for (int b = 0; b < table.length; ++b) {
				for (Entry<T> entry = table[b]; entry != null; entry = entry.next) {
					result[scan++] = entry.hash;
				}
			}
	...
	private static int hash(long key, int mod) {
		return (int) ((key & 0x7FFFFFFFFFFFFFFFL) % mod);
	}
}
